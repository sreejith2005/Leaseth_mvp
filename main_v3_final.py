"""
V3 Model Training - Financial Only (No Evictions)
...
MODEL_XGBOOST_PATH = "models/xgboost_model_financial.pkl"
MODEL_LIGHTGBM_PATH = "models/lightgbm_model_financial.pkl"
FEATURES_PATH = "models/feature_list_financial.pkl"
CALIBRATION_PATH = "models/calibration_model_financial.pkl"
TARGET = 'default_flag'
...
def main():
    print("=" * 80)
    print("V3 MODEL TRAINING - FINANCIAL ONLY (NO EVICTIONS)")
    ...
    exclude_cols = leaky_features + raw_cats + [TARGET, 'previous_evictions', 'eviction_binary',
                                                  'number_of_bedrooms', 'property_size_sqft', 
                                                  'property_age', 'currency_enc']

    v3_features = [col for col in df_feat.columns if col not in exclude_cols]
    
    print(f"\nV3 feature count: {len(v3_features)}")
    print("Strategy: Financial features only, no eviction history")
    print("Class imbalance handling: scale_pos_weight (no SMOTE)")

    X = df_feat[v3_features]
    y = df_feat[TARGET]
    ...
    y_pred_lgb = lgb_model.predict(X_test)  # type: ignore
